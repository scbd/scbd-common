    ...
    <div class="km-nav-wizard">
      ...
      <div class="nav-header mb-2">
        <ul class="nav nav-pills nav-fill">
          <li @click.prevent.stop="selectTab(index)" :class="{ 'active': tab.isActive }" v-for="(tab, index) in tabs" :key="`tab-${index}`">
      ...
      <div class="nav-body">
        <form>
          <div>SADSADASDSADSADSADASDSADSAD</div>
          <slot></slot>
        </form>
      </div>
...
const currentTab = computed(() => tabs.value.find(e => e.isActive));
// const activeTabIndex = ref(1);
console.log("totalTabs", totalTabs)
// console.log("currentTab", activeTabIndex)
