import { ref, onMounted, onBeforeUnmount, openBlock, createElementBlock, createElementVNode, unref, Fragment, renderList, toDisplayString, createTextVNode, createCommentVNode, pushScopeId, popScopeId } from 'vue';

const _withScopeId = n => (pushScopeId("data-v-a185aa92"),n=n(),popScopeId(),n);
const _hoisted_1 = { class: "countdown" };
const _hoisted_2 = { class: "alignment" };
const _hoisted_3 = ["href"];
const _hoisted_4 = ["src"];
const _hoisted_5 = { class: "alignment" };
const _hoisted_6 = ["href"];
const _hoisted_7 = { key: 0 };
const _hoisted_8 = { class: "countdown-box" };
const _hoisted_9 = { key: 0 };
const _hoisted_10 = /*#__PURE__*/ _withScopeId(() => /*#__PURE__*/createElementVNode("br", null, null, -1 /* HOISTED */));
const _hoisted_11 = { key: 1 };


var script = {
  __name: 'cop-count-down',
  props: {
  copTitle: {type: String, required: true},
  endDate:  {type: Date,   required: true},
  url:      {type: String, required: false},
  imageSource:{type: String, required: false}
  
},
  setup(__props) {

const { copTitle, endDate, url, imageSource } = __props;

const currentDate = ref(new Date());
const timer = ref(null);
let imgSrc = '';


const getNumberDays = () => {
  const countDownDate = endDate.getTime();
  const now = currentDate.value.getTime();
  const distance = countDownDate - now;
  const days = Math.ceil(distance / (1000 * 60 * 60 * 24)).toString().padStart(2, "0");
  const hours = Math.ceil((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)).toString().padStart(2, "0");
  const minutes = Math.ceil((distance % (1000 * 60 * 60)) / (1000 * 60)).toString().padStart(2, "0");
  const seconds = Math.ceil((distance % (1000 * 60)) / 1000).toString().padStart(2, "0");

  return { days, hours, minutes, seconds };
};

const time = getNumberDays();

onMounted(() => {
  timer.value = setInterval(() => {
    currentDate.value = new Date();
  }, 1000 * 60);
});

onBeforeUnmount(() => {
  clearInterval(timer.value);
});
    //An image should be passed in props
    if(imageSource){
      imgSrc = imageSource;
    }
    else {
    imgSrc = 'data:image/png;base64,';
    }

return (_ctx, _cache) => {
  return (openBlock(), createElementBlock("section", _hoisted_1, [
    createElementVNode("div", _hoisted_2, [
      createElementVNode("a", { href: __props.url }, [
        createElementVNode("img", {
          width: "35",
          height: "35",
          alt: "",
          src: unref(imgSrc)
        }, null, 8 /* PROPS */, _hoisted_4)
      ], 8 /* PROPS */, _hoisted_3)
    ]),
    createElementVNode("div", _hoisted_5, [
      createElementVNode("a", { href: __props.url }, [
        (unref(time).days >= 1)
          ? (openBlock(), createElementBlock("span", _hoisted_7, [
              (openBlock(true), createElementBlock(Fragment, null, renderList(unref(time).days, (char) => {
                return (openBlock(), createElementBlock("span", { key: char }, [
                  createElementVNode("span", _hoisted_8, toDisplayString(char), 1 /* TEXT */),
                  createTextVNode("  ")
                ]))
              }), 128 /* KEYED_FRAGMENT */)),
              createTextVNode("DAY"),
              (unref(time).days > 1)
                ? (openBlock(), createElementBlock("span", _hoisted_9, "S"))
                : createCommentVNode("v-if", true),
              _hoisted_10,
              createTextVNode("To " + toDisplayString(__props.copTitle), 1 /* TEXT */)
            ]))
          : (openBlock(), createElementBlock("span", _hoisted_11, toDisplayString(__props.copTitle), 1 /* TEXT */))
      ], 8 /* PROPS */, _hoisted_6)
    ])
  ]))
}
}

};

script.__scopeId = "data-v-a185aa92";
script.__file = "src/components/countdown/cop-count-down.vue";

export { script as default };
//# sourceMappingURL=cop-count-down.js.map
