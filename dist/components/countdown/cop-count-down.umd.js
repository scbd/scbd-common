(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('vue')) : typeof define === 'function' && define.amd ? define(['vue'], factory) : (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.ScbdCommonComponentsCountdownCopCountDown = factory(global.vue));
})(this, function (vue) {
  'use strict';

  const _withScopeId = n => (vue.pushScopeId("data-v-a185aa92"), n = n(), vue.popScopeId(), n);
  const _hoisted_1 = {
    class: "countdown"
  };
  const _hoisted_2 = {
    class: "alignment"
  };
  const _hoisted_3 = ["href"];
  const _hoisted_4 = ["src"];
  const _hoisted_5 = {
    class: "alignment"
  };
  const _hoisted_6 = ["href"];
  const _hoisted_7 = {
    key: 0
  };
  const _hoisted_8 = {
    class: "countdown-box"
  };
  const _hoisted_9 = {
    key: 0
  };
  const _hoisted_10 = /*#__PURE__*/_withScopeId(() => /*#__PURE__*/vue.createElementVNode("br", null, null, -1 /* HOISTED */));
  const _hoisted_11 = {
    key: 1
  };
  var script = {
    __name: 'cop-count-down',
    props: {
      copTitle: {
        type: String,
        required: true
      },
      endDate: {
        type: Date,
        required: true
      },
      url: {
        type: String,
        required: false
      },
      imageSource: {
        type: String,
        required: false
      }
    },
    setup(__props) {
      const {
        copTitle,
        endDate,
        url,
        imageSource
      } = __props;
      const currentDate = vue.ref(new Date());
      const timer = vue.ref(null);
      let imgSrc = '';
      const getNumberDays = () => {
        const countDownDate = endDate.getTime();
        const now = currentDate.value.getTime();
        const distance = countDownDate - now;
        const days = Math.ceil(distance / (1000 * 60 * 60 * 24)).toString().padStart(2, "0");
        const hours = Math.ceil(distance % (1000 * 60 * 60 * 24) / (1000 * 60 * 60)).toString().padStart(2, "0");
        const minutes = Math.ceil(distance % (1000 * 60 * 60) / (1000 * 60)).toString().padStart(2, "0");
        const seconds = Math.ceil(distance % (1000 * 60) / 1000).toString().padStart(2, "0");
        return {
          days,
          hours,
          minutes,
          seconds
        };
      };
      const time = getNumberDays();
      vue.onMounted(() => {
        timer.value = setInterval(() => {
          currentDate.value = new Date();
        }, 1000 * 60);
      });
      vue.onBeforeUnmount(() => {
        clearInterval(timer.value);
      });
      //An image should be passed in props
      if (imageSource) {
        imgSrc = imageSource;
      } else {
        imgSrc = 'data:image/png;base64,';
      }
      return (_ctx, _cache) => {
        return vue.openBlock(), vue.createElementBlock("section", _hoisted_1, [vue.createElementVNode("div", _hoisted_2, [vue.createElementVNode("a", {
          href: __props.url
        }, [vue.createElementVNode("img", {
          width: "35",
          height: "35",
          alt: "",
          src: vue.unref(imgSrc)
        }, null, 8 /* PROPS */, _hoisted_4)], 8 /* PROPS */, _hoisted_3)]), vue.createElementVNode("div", _hoisted_5, [vue.createElementVNode("a", {
          href: __props.url
        }, [vue.unref(time).days >= 1 ? (vue.openBlock(), vue.createElementBlock("span", _hoisted_7, [(vue.openBlock(true), vue.createElementBlock(vue.Fragment, null, vue.renderList(vue.unref(time).days, char => {
          return vue.openBlock(), vue.createElementBlock("span", {
            key: char
          }, [vue.createElementVNode("span", _hoisted_8, vue.toDisplayString(char), 1 /* TEXT */), vue.createTextVNode("  ")]);
        }), 128 /* KEYED_FRAGMENT */)), vue.createTextVNode("DAY"), vue.unref(time).days > 1 ? (vue.openBlock(), vue.createElementBlock("span", _hoisted_9, "S")) : vue.createCommentVNode("v-if", true), _hoisted_10, vue.createTextVNode("To " + vue.toDisplayString(__props.copTitle), 1 /* TEXT */)])) : (vue.openBlock(), vue.createElementBlock("span", _hoisted_11, vue.toDisplayString(__props.copTitle), 1 /* TEXT */))], 8 /* PROPS */, _hoisted_6)])]);
      };
    }
  };
  script.__scopeId = "data-v-a185aa92";
  script.__file = "src/components/countdown/cop-count-down.vue";
  return script;
});
//# sourceMappingURL=cop-count-down.umd.js.map
